...
const ARTIST = {
  name: "מרין אטיה זוהר",
  subtitle: "אומנות רב-תחומית", // תת-כותרת קטנה מתחת לשם
  phone: "054-7331315",
};
...
const HERO_STORY = [
  "האהבה שלי לאמנות וליצירה פועם בי מילדות, כילדה שגדלה לצד הציורים מעוררי ההשראה של אמי. היד שלי תמיד חיפשה דף וצבעים, וציירתי בכל הזדמנות. כעת, בנקודת הזמן הנוכחית בחיי, בחרתי לחזור הביתה - אל הצבעים, החומרים והמכחולים. החלטתי להציב את האמנות במרכז הבמה של חיי, להעניק לה את המקום הראוי לה, להתפתח ולצמוח יחד איתה.",
];

// פסקה שמופיעה מתחת לקרוסלה (באותו סקשן, אותו רקע)
const GALLERY_NOTE =
  "היצירות שלי הן שיקוף של עולמי הפנימי, ומבטאות בין היתר חיבור עמוק ובלתי נפרד לטבע, לים (חיבור שמתבקש עם שם כמו שלי) וליצורים החיים בו. וכן אני שואבת השראה רבה מאינספור טיולים שעשיתי ברחבי העולם, הממלאים אותי בטקסטורות, גוונים וסיפורים שמקבלים חיים חדשים ביצירות שלי.";
...
const FEATURE_1 = {
  quote:
    "אני יוצרת מתוך רצון לגעת בלבבות, ולהעשיר את המרחבים בחיינו בצבעים עזים, חיים ואופטימיים. עבורי, כל עבודה היא סיפור חד-פעמי שנולד כדי להעניק מתנה מרגשת ועתירת מחשבה – בין אם עבור האנשים היקרים לי, ובין אם עבור אלו מכם שמחפשים להעניק לאדם חשוב בחייהם (או לעצמם) פריט אמנות ייחודי עם נשמה.",
  signature: "",
  art: "/feature-1.png",
  w: 1312,
  h: 1199,
...
const FEATURE_2 = {
  quote:
    "היצירות שלי מתפרשות על פני מגוון מדיומים משתנים: החל מציורי קנבס עזים וכדים צבועים בעבודת יד, ועד ליצירות פופ-ארט מודרניות ומפתיעות, המשלבות קוביות לגו יחד עם חומרים מהטבע או חומרים ממוחזרים – כמו עציצי לגו ייחודיים, המייצרים חיבור חדש ומרתק בעין.",
  signature: "",
  art: "/feature-2.png",
  w: 1254,
  h: 1254,
  ...
  return (
    <main>
      ...
      <section className="section" aria-labelledby="gallery-title">
        ...
        <div className="container gallery__note">
          <p className="quote quote--center">{GALLERY_NOTE}</p>
        </div>
      </section>
